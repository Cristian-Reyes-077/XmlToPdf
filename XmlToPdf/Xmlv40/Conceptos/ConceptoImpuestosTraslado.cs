...
namespace XmlToPdf.Xmlv40.Conceptos
{
    ...
    public partial class ConceptoImpuestosTraslado
    {
        ...
        /*public string guardarElem(int idconcepto)
        {
            string qry = "";
            List<object> listaColsVal = FuncionesG.qryInsert(this);
            if (listaColsVal.Count > 0)
            {
                qry = $"{FuncionesG.ListaObjectToqry(listaColsVal, "insert into cfd_concepto_impuestos(concepto_id,Tipo,", $"values({idconcepto},'T','")}";
            }
            return qry;
        }*/
